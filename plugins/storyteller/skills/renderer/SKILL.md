...
필수 입력:
- 승인된 카드 구성 (planner 출력: 카드 타입, 카드별 카피 텍스트)
...
1. 사용할 카드 타입에 따라 템플릿 파일 읽기
   - `card-gradient`, `card-white` → `assets/full-image-case.html`
   - `card-image-overlay`, `card-image-top` → `assets/part-image-case.html`
